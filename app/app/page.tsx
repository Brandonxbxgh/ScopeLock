...
export default function App() {
  ...
  const blockedProjects = useMemo(() => {
    return projects.filter(project => {
      ...
      return statusInfo && statusInfo.status === 'Blocked';
    });
  }, [projects, projectStatuses]);

  // Sort projects: Blocked → In Progress → Planning → Completed
  const sortedProjects = useMemo(() => {
    ...
    return [...projects].sort((a, b) => {
      const statusA = projectStatuses[a.id]?.status || 'Planning';
      const statusB = projectStatuses[b.id]?.status || 'Planning';
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-semibold text-red-800 dark:text-red-300">
                  {blockedProjects.length === 1 ? 'Project Blocked' : 'Projects Blocked'}
                </h3>
                <div className="mt-1 text-sm text-red-700 dark:text-red-400">
                  {blockedProjects.length === 1 ? (
                    <>
                      <button
                        onClick={() => router.push(`/projects/${blockedProjects[0].id}`)}
                        className="font-medium underline hover:no-underline"
                      >
                        {blockedProjects[0].name}
                      </button>
                      {' '}has reached its feature limit. Mark features as done to continue.
                    </>
                  ) : (
                    <>
